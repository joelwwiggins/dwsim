<script lang="ts">
	...
	interface EquilibriumReactionData {
		id: string;
		name?: string;
	...
	interface EquilibriumReactorData {
		label: string;
		unitType: string;
		operation_mode: 'isothermal' | 'adiabatic';
		target_temperature?: number;
		pressure_drop: number;
		reactions: EquilibriumReactionData[];
	}

	interface Props {
		unit: EquilibriumReactorData;
		isOpen: boolean;
		onClose: () => void;
		onSave: (data: EquilibriumReactorData) => void;
	}

	let { unit, isOpen, onClose, onSave }: Props = $props();

	let editedUnit: EquilibriumReactorData = $state(null);
	...
	const availableCompounds = [
		...
		{ id: 'sulfur_dioxide', name: 'Sulfur Dioxide', formula: 'SO₂' },
		{ id: 'sulfur_trioxide', name: 'Sulfur Trioxide', formula: 'SO₃' }
	];
	...
	function addReaction() {
		if (editedUnit) {
			const newReaction: EquilibriumReactionData = {
				id: `reaction_${Date.now()}`,
				name: `Equilibrium Reaction ${editedUnit.reactions.length + 1}`,
				stoichiometry: {},
	...
	<div class="modal-overlay" onclick={onClose}>
		<div class="modal-content" onclick={(e) => e.stopPropagation()}>
			<div class="modal-header">
				<h2>Equilibrium Reactor Properties</h2>
			...
			<div class="modal-body">
								...
								<div class="reaction-header">
									...
									<div class="reaction-controls">
										...
										<input
											...
											min="0"
											step="0.001"
											bind:value={reaction.equilibrium_constant}
											class="keq-input"
				...
				<div class="info-section">
					<p><strong>Note:</strong> Coefficients should be negative for reactants and positive for products. The equilibrium constant K_eq determines how far the reaction proceeds toward equilibrium.</p>
					<p><strong>Current limitation:</strong> This simplified implementation handles single reactions. Full equilibrium reactor would solve multiple coupled reactions simultaneously.</p>
				</div>
			</div>
