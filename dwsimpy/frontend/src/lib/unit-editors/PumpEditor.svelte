<script lang="ts">
	...
	$effect(() => {
		if (unit) {
			...
			editedUnit = {
				...
				name: '',
				outlet_pressure: 200,
				pressure_increase: 100,
				efficiency: 75,
				power_consumption: 10,
				calculation_mode: 'pressure_increase',
				pressure_unit: 'kPa',
				power_unit: 'kW'
			};
		}
	...
	function updateCalculationMode(mode: 'outlet_pressure' | 'pressure_increase' | 'power_consumption') {
		if (editedUnit) {
		...
		<div class="modal-content" onclick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" tabindex="-1">
			...
			<div class="modal-body">
				...
				<div class="property-section">
					...
					<div class="radio-group">
						...
						<label class="radio-option">
							<input
								type="radio"
								value="pressure_increase"
								bind:group={editedUnit.calculation_mode}
								onchange={() => updateCalculationMode('pressure_increase')}
							/>
							<span>Specify Pressure Increase</span>
						</label>
						<label class="radio-option">
							<input
								type="radio"
								...
								bind:group={editedUnit.calculation_mode}
								onchange={() => updateCalculationMode('power_consumption')}
							/>
							<span>Specify Power Consumption</span>
						</label>
					</div>
				</div>
				...
				<div class="property-section">
					...
					{:else if editedUnit.calculation_mode === 'pressure_increase'}
						<div class="form-row">
							<label for="pressure-increase">Pressure Increase:</label>
							<div class="input-group">
								<input
									id="pressure-increase"
									type="number"
									step="0.1"
									bind:value={editedUnit.pressure_increase}
								/>
								<select bind:value={selectedPressureUnit}>
									{#each pressureUnits as unit}
										<option value={unit}>{unit}</option>
						...
						<div class="form-row">
							...
							<div class="input-group">
								<input
									...
									type="number"
									step="0.1"
									bind:value={editedUnit.power_consumption}
								/>
								<select bind:value={selectedPowerUnit}>
									{#each powerUnits as unit}
										<option value={unit}>{unit}</option>
									{/each}
								</select>
							</div>
