...
class Pump(BaseUnitOperation):
    ...
    def __init__(self, unit_id: str, config: Dict[str, Any]):
        ...
        self.outlet_pressure = config.get('outlet_pressure')  # Optional target pressure
    ...
    def solve(self) -> float:
        ...
        # If outlet pressure is specified, calculate pressure rise
        if self.outlet_pressure is not None:
            self.pressure_rise = self.outlet_pressure - inlet_stream.pressure
        else:
            self.outlet_pressure = inlet_stream.pressure + self.pressure_rise
