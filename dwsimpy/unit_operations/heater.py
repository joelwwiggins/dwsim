"""
Heater unit operation for DWSIM Python implementation.
"""
...
class Heater(BaseUnitOperation):
    """Heater unit operation - adds heat to a stream"""
    ...
    def solve(self) -> float:
        """Solve the heater equations"""
        if len(self.inlet_streams) != 1:
            raise ValueError("Heater requires exactly 1 inlet stream")

        if len(self.outlet_streams) != 1:
            raise ValueError("Heater requires exactly 1 outlet stream")
        ...
        self.results = {
            ...
            'temperature_rise': outlet_temp - inlet_stream.temperature,
            'pressure_drop': self.pressure_drop
        }

        return 0.0  # Perfect convergence for simple heater